...
function pinchIntensity(landmarks: Landmark[]): number {
  ...
  return span > 0
    ? Math.max(0, Math.min(1, 1 - raw / span))
    : 0;
}
...
export class HandTracker {
  ...
  private loop = (): void => {
    ...
    for (let i = 0; i < result.landmarks.length; i++) {
      ...
      const intensity = pinchIntensity(
        result.landmarks[i] as Landmark[],
      );

      const mirrored = result.landmarks[i].map((lm) => ({
        ...lm,
        x: 1 - lm.x,
      })) as Landmark[];
  ...
  private extractPinch(
    ...
    for (let i = 0; i < result.landmarks.length; i++) {
      ...
      if (label === 'left') {
        state.right = intensity;
      }

      if (label === 'right') {
        state.left = intensity;
      }
    }
